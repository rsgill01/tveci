...
r.tvec=function(data, otherX=NULL, lag=1, nbeta=50, ngamma=50, trim=0.05){
 n=nrow(data)
...
ECTi*(1-d),1-d,dX*(1-d),otherX[(lag+1):(n-1),]*(1-d))
  A.hat=try(solve(t(Z)%*%Z)%*%t(Z)%*%dY,silent=TRUE)
  list(beta=beta.hat,gamma=gamma.hat,A=A.hat,lag=lag,res=dY-Z%*%A.hat,min.rss=min.RSS)
 }
 else{
...
c.tvec=function(data, otherX=NULL, lag=1, nbeta=50, ngamma=50, trim=0.05){
 ...
 d=data[,1:2]
 out=.C("tvec", as.double(d), as.double(otherX), n, l, nb, ng, trm, cX, beta=double(1), gamma=double(1), A=double(2*(4+4*l+2*cX)), res=double(2*(n-l-1)), min.RSS=double(1))
 if ((!is.na(out$min.RSS))&(out$min.RSS >= 0)){
  list(beta=out$beta, gamma=out$gamma, A=matrix(out$A,ncol=2), lag=lag, res=matrix(out$res,ncol=2),min.rss=out$min.RSS)
 }
 else{
  stop("system is computationally singular\n")
 }
...
r.tveci=function(data, lag=1, icount.lim=5, count.lim=100, ep=1e-4, otherX=NULL,...){
 z=which(data[,3]==0)
 zz=which(data[,3]==1)
 p=data[,1:2]
 n=nrow(p)
...
c.tveci=function(data, lag=1, icount.lim=5, count.lim=100, ep=1e-4, otherX=NULL, ...){
 z=which(data[,3]==0)
 zz=which(data[,3]==1)
 p=data[,1:2]
 n=nrow(p)
